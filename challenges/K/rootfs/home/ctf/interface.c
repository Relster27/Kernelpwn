...
void signup(uint8_t **data, int index){
    ...
    len = sprintf(cookie, "{\"username\": \"%s\", \"admin\": %s}", username, "true");
...
int main(int argc, char*argv[]){
    ...
    if((data = mmap((void*)0xdeadbeef000, 0x1000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0)) == MAP_FAILED) err("error: mmap failed");
